  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Table>
            ...
            <Tbody>
              ...
              {!data.recent_payments.length && (
                <Tr><Td colSpan={4} className="text-center text-gray-400 py-8">No payments yet</Td></Tr>
              )}
            </Tbody>
          </Table>
        </div>
        ...
        <div className="card">
          <div className="p-4 border-b">
            <h3 className="font-semibold text-gray-900">Members Expiring Soon</h3>
          </div>
          <Table>
            <Thead>
              <Th>Member</Th>
            ...
            <Tbody>
              {data.members_expiring_soon.map((m) => (
                <Tr key={m.id}>
                  ...
                  <Td>{m.phone}</Td>
                  <Td className="text-orange-600 font-medium">{new Date(m.expiry_date).toLocaleDateString('en-PK')}</Td>
                </Tr>
              ))}
              {!data.members_expiring_soon.length && (
                <Tr><Td colSpan={3} className="text-center text-gray-400 py-8">No members expiring soon</Td></Tr>
              )}
            </Tbody>
          </Table>
        </div>
